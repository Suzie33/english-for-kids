        ...
        for (let i = 0; i < cardsWords.length - 1; i++) {
          for (let j = 0; j < cardsWords[i].length; j++) {
            if (cardsWords[i][j].word === card.word) {
              ...
              cardsWords[i][j]['%'] = parseInt((1 - cardsWords[i][j].errors / (cardsWords[i][j].correct + cardsWords[i][j].errors)) * 100, 10);
  ...
  loadStatsPage() {
    ...
    cardsWords.forEach((group) => {
      group.forEach((word) => {
        ...
        word['%'] = parseInt((1 - word.errors / (word.correct + word.errors)) * 100, 10);
